...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($erreurs)) {
        $stmt = $pdo->prepare("
            INSERT INTO affectation (annee_scolaire, id_classe, id_enseignant, id_matiere)
            VALUES (:annee_scolaire, :id_classe, :id_enseignant, :id_matiere)
        ");
        $stmt->execute([
...
<div class="container mt-4" style="max-width: 550px;">
    ...
    <h1 class="h3 mb-4">🔗 Nouvelle affectation</h1>
            ...
            <form method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="annee_scolaire" class="form-control"
                           ...
                           value="<?= htmlspecialchars($_POST['annee_scolaire'] ?? '') ?>" required>
                </div>
                    ...
                    <select name="id_enseignant" class="form-select" required>
                        ...
                        <?php foreach ($enseignants as $e): ?>
                            <option value="<?= $e['id_enseignant'] ?>"
                                <?= (($_POST['id_enseignant'] ?? 0) == $e['id_enseignant']) ? 'selected' : '' ?>>
                    ...
                    <select name="id_matiere" class="form-select" required>
                        ...
                        <?php foreach ($matieres as $m): ?>
                            <option value="<?= $m['id_matiere'] ?>"
                                <?= (($_POST['id_matiere'] ?? 0) == $m['id_matiere']) ? 'selected' : '' ?>>
